...
function TFSchedule(config) {
    ...
    if (!(backExecRecordNum && mysqlConfig && taskRootPath && notifyList)) {
        return this.throwError(
            "please init with enough params ，need backExecRecordNum && mysqlConfig && taskRootPath && notifyList to init"
        );
    }
...
TFSchedule.prototype = {
    ...
    init: function*() {
        ...
        try {
            ...
            this.throwError(
                "check database has t_task_list,t_task_exec_list failed",
                e
            );
        }
    },
    ...
    startSystem: function*() {
        var { taskRuleMap, notifyList, mysqlClient } = this;
        var that = this;
        ...
        // 2. 查询数据库中所有任务
        var taskListRes = yield mysqlClient.query(
            "select taskName,rule from t_task_list"
        );
        this.emit("notify", {
            type: "systemNotify",
            notifyList,
