...
function* execTask(taskName) {
    ...
    try {
        // 1: 数据库查询该条任务,
        const data = yield mysqlClient.query(
            `select * from t_task_list where taskName='${taskName}'`
        );
        if (data.length === 0) {
            this.throwError(
                `(${taskName})-任务在数据库中已被删除，不再执行该任务`
            );
            return false;
        }
        const taskInfo = data[0];
        const { taskStatus, title } = taskInfo;
        ...
        if (taskStatus !== 0) {
            console.log(
                `(${taskName})-任务被设置为无效状态码taskStatus:${taskStatus}，跳过，不执行`
            );
            return false;
        }
        // 3. 使用子进程spawn接口运行任务
...
function* spawnTask(taskInfo) {
    ...
    const taskLogFile = path.join(
        taskRootPath,
        taskName,
        `logs/${taskVersion}.log`
    );
    ...
    console.log(
        `执行任务${taskName}-${taskPid}: ${command} ${taskExecFilePath}`
    );
    ...
    childProcessHandleCache[taskName].stdout.on("data", function(data) {
        var logItem = `${moment().format(
            "YYYY-MM-DD HH:mm:ss"
        )} stdout-${taskPid}: ${new Buffer(data).toString()}`;
        fs.appendFileSync(taskLogFile, logItem);
    });

    // stderr输出到任务日志
    childProcessHandleCache[taskName].stderr.on("data", function(data) {
        var logInfo = `${moment().format(
            "YYYY-MM-DD HH:mm:ss"
        )} stderr-${taskPid} ${new Buffer(data).toString()}`;
